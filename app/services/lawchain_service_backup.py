...
class LawChainService:
    ...
    def initialize_langchain(self, force_rebuild: bool = False):
        ...
        try:
            ...
            # Import dan inisialisasi LangChain implementation (OPTIMIZED VERSION)
            from .lawchain_optimized import OptimizedLawChainIndonesia
            
            self.langchain_instance = OptimizedLawChainIndonesia()
            self.langchain_instance.initialize_optimized(force_rebuild_vectorstore=force_rebuild)
            
            self.initialization_status['langchain'] = True
            logger.info("Optimized LangChain implementation initialized successfully")
            
        except Exception as e:
            logger.error(f"Failed to initialize Optimized LangChain implementation: {str(e)}")
        ...
        try:
            if method == "langchain":
                if not self.initialization_status['langchain'] or not self.langchain_instance:
                    raise ValueError("Optimized LangChain implementation not initialized")
                
                logger.info(f"Processing question with Optimized LangChain: {question[:50]}...")
                response = self.langchain_instance.ask_question_optimized(question)
